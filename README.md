...
EndoriumFort follows an **open-core** model:

- **Community Edition (CE)** — this repository — is released under the
  **EndoriumFort Community Edition Source-Available License v1.1** (see
  [LICENSE](LICENSE)).
  - **Free** for personal, educational, research, evaluation, and internal
    non-production use.
  - **Commercial or production use** requires an **Enterprise license**.
  - Verbatim redistribution is allowed with attribution; modified versions may
    only be contributed back via pull request.
- **Enterprise Edition (EE)** — premium features (SSO/LDAP/SCIM, HA/cluster,
  relays, RDP/VNC, session recording & DNA, evidence packs, JIT governance,
  security center, agent tunneling, and more) — is a separate, license-gated
...
Contributions via pull requests are welcome! Modified versions may only be
distributed as pull requests back to the original repository (see LICENSE §5).
