...
int main() {
  CrowApp app;
  AppContext ctx;
  const RuntimeConfig runtime_config = load_runtime_config(ctx);
  apply_runtime_config(ctx, runtime_config);
  ...
  // ── License (freemium open-core): load + verify offline, then expose to gate ──
  ctx.reload_license();
  g_license_ctx = &ctx;
