...
// Security Center routes now live in the pro/ overlay (pro/security_center.cc).
void register_pro_security_routes(CrowApp &app, AppContext &ctx);
...
bool test_auth_failure_burst_alert() {
  ...
  register_pro_security_routes(app, ctx);
  app.validate();
